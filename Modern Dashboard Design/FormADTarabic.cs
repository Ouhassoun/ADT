...
{
    public partial class FormADTarabic : Form
    {
        SQLiteConnection cnx = new SQLiteConnection(@"URI=file:" + Application.StartupPath + "\\ADTDB.db;PRAGMA journal_mode = WAL;");
        private DataView dataView;
        public FormADTarabic()
        {
            InitializeComponent();
        }
        private void FormADTarabic_Load(object sender, EventArgs e)
        {
            DGVUpdate();
        }
        ...
        public bool Exists(string cin)
        {
            ...
            SQLiteCommand cmd = new SQLiteCommand("select * from userAR where CIN=@cin", cnx);
        ...
        {
            AttestationDuTravailAR attestationDuTravail = new AttestationDuTravailAR();
        ...
        void DGVUpdate()
        {
            SQLiteCommand cmd = new SQLiteCommand("select * from userAR", cnx);
            cnx.Open();
            SQLiteDataReader dr = cmd.ExecuteReader();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                SQLiteCommand cmd = new SQLiteCommand("insert into userAR values(@num,@nometprenom,@cin,@matricule,@dateRec,@grade,@fonction,@affectation)", cnx);
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            if (txtcin.Text != "")
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand("select * from userAR where upper(CIN) = upper(@cin)", cnx);
                cmd.Parameters.AddWithValue("@cin", txtcin.Text);
                cnx.Open();
                SQLiteDataReader dr = cmd.ExecuteReader();
                if (dr.HasRows)
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (!IsEmpty() && Exists(txtcin.Text))
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand("delete from userAR where CIN=@cin", cnx);
                cmd.Parameters.AddWithValue("@cin", txtcin.Text);
                cnx.Open();
                cmd.ExecuteNonQuery();
                cnx.Close();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (!IsEmpty())
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand("update userAR set Numero=@num,Nometprenom=@nometprenom,Matricule=@matricule,DateDeRecru=@dateRec,Grade=@grade,Fonction=@fonction,Affectation=@affectation where CIN=@cin", cnx);
                cmd.Parameters.AddWithValue("@num", txtnum.Text);
                cmd.Parameters.AddWithValue("@cin", txtcin.Text);
                cmd.Parameters.AddWithValue("@nometprenom", txtname.Text);
                cmd.Parameters.AddWithValue("@matricule", txtmat.Text);
